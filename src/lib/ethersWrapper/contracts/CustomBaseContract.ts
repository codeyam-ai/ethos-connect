...
import interceptTransaction from "./interceptTransaction";
...
function buildSend(contract: Contract, fragment: FunctionFragment): ContractFunction<TransactionResponse> {
    return async function(...args: Array<any>): Promise<TransactionResponse> {
        ...
        const shouldCancel = await interceptTransaction(contract, txRequest, fragment);
        console.log('shouldCancel :>> ', shouldCancel);
        
        if (!shouldCancel) {
            // ETHOS NOTE: Call through
        ...
        // ETHOS NOTE: interceptTransaction should return the TransactionResponse so this library can continue
        const fakeTx: TransactionResponse = {
            hash: "",
            confirmations: 0,
            from: "",
            wait: function (confirmations?: number | undefined): Promise<TransactionReceipt> {
                console.log('fakeTx wait()');
                ...
                return Promise.resolve(fakeTxReceipt)
            },
            nonce: 0,
...
export class CustomBaseContract {
    readonly address: string;
    readonly interface: Interface;
